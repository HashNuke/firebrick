...
# %% and send Message to the client.
  @spec init(binary, non_neg_integer, tuple, list), do: {:ok, string, State} | {:stop, any, string}
  ...
  @spec handle_HELO(binary, State[]), do: {:ok, pos_integer, State} | {:ok, State} | error_message
...
#   {ok, Reference, State}.
  ...
  @spec handle_RSET(State), do: State
  def handle_RSET(state) do
    # reset any relevant internal state
    state
  end

  @spec handle_VRFY(binary, State), do: {:ok, string, State} | {:error, string, State}
  ...
  @spec handle_other(binary, binary, State), do: {string, State}
...
#   error.
  ...
  @spec handle_STARTTLS(State), do: State
  def handle_STARTTLS(state) do
    :io.format("TLS Started~n")
    state
  end

  @spec code_change(any, State, any), do: {:ok, State}
  def code_change(_old_vsn, state, _extra) do
    {:ok, state}
  end

  @spec terminate(any, State), do: {:ok, any, State}
